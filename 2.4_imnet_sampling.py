"""
2.4_imnet_sampling.py

이 스크립트는 2.2_floodfill.py로 생성된 model_filled.binvox 파일에서 다양한 해상도의 포인트 샘플과 값을 추출하여 HDF5 파일로 저장합니다.

[동작 설명]
- input_dir/shapes/ 하위의 각 폴더에 있는 model_filled.binvox 파일을 읽어 포인트 샘플링을 수행합니다.
- 16, 32, 64 해상도별로 포인트와 값을 추출하여 ae_voxel_points_samples.hdf5로 저장합니다.
- input_dir/names.npz 파일을 참조하여 처리할 파일 리스트를 만듭니다.
- 멀티프로세싱으로 빠르게 처리합니다.

[입력]
- input_dir: shapes 폴더가 포함된 상위 폴더 (예: /workspace/data/cylinder_data)
- input_dir/shapes/ 하위에 model_filled.binvox 파일들이 있어야 함
- input_dir/names.npz 파일이 있어야 함

[출력 및 저장 구조]
- input_dir/ae_voxel_points_samples.hdf5 (HDF5 파일)
    ├─ voxels: (N, 64, 64, 64, 1)  # 각 모델의 64³ 복셀 (uint8)
    ├─ points_16: (N, 4096, 3)     # 16³ 해상도 샘플 포인트 좌표 (uint8)
    ├─ values_16: (N, 4096, 1)     # 16³ 해상도 샘플 포인트 값 (uint8)
    ├─ points_32: (N, 8192, 3)     # 32³ 해상도 샘플 포인트 좌표 (uint8)
    ├─ values_32: (N, 8192, 1)     # 32³ 해상도 샘플 포인트 값 (uint8)
    ├─ points_64: (N, 24576, 3)    # 64³ 해상도 샘플 포인트 좌표 (uint8)
    └─ values_64: (N, 24576, 1)    # 64³ 해상도 샘플 포인트 값 (uint8)
    ※ N은 모델 개수이며, 각 데이터셋은 names.npz의 순서와 일치

- input_dir/statistics.txt (텍스트 파일)
    - 전체 모델 개수, 샘플링 초과 건수 등 통계 정보가 텍스트로 저장됨

[실행 인자]
- --input_dir (str, 기본값 '/workspace/data/cylinder_data'): shapes 폴더가 포함된 상위 폴더 경로

[예시 실행]
python 2.4_imnet_sampling.py --input_dir /workspace/data/cylinder_data
# 또는 기본값 사용 시
python 2.4_imnet_sampling.py
"""
import numpy as np
import os, sys
...
parser.add_argument("--input_dir", type=str, default="/workspace/data/cylinder_data", help="shapes 폴더가 포함된 상위 폴더 경로 [기본값: /workspace/data/cylinder_data]")
...
if __name__ == '__main__':
	...
	# voxel_input = os.path.join(input_dir)
	voxel_input = os.path.join(input_dir, 'shapes')
 	# voxel_input = input_dir
	...
	name_list = np.load(os.path.join(input_dir, 'names.npz'))['names']
