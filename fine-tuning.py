...
def main_function(start_index, end_index, experiment_directory, 
	...
	# epoches_each_stage = 500 #more is better, optional 500 is more stable than 300
	# epoches_each_stage = 1000 #more is better, optional 500 is more stable than 300
	stages = [0, 1, 2]
	...
	for index in shape_indexes:
		...
		for phase in stages:
			...
			if phase == 0:
				continue_from = args.init_weight
...
if __name__ == "__main__":
	...
	arg_parser.add_argument("--leaky", dest="leaky", action="store_false", help="soft min max")
	arg_parser.add_argument("--noweight", dest="noweight", action="store_true", help="make left op and right op the same weight or not option")
	arg_parser.add_argument("--test", dest="test", default=True, help="test or train option")
	arg_parser.add_argument("--grid_sample", dest="grid_sample", default=64, help="dataset option")
	arg_parser.add_argument("--start", dest="start_index", default=0, help="finetuning start index")
	arg_parser.add_argument("--end", dest="end_index", default=1, help="finetuning end_index")
	arg_parser.add_argument("--surface", dest="surface", action="store_true", help="point cloud option")
	arg_parser.add_argument("--voxel", dest="voxel", default=True, help="voxel option")
	arg_parser.add_argument("--shapenet_flag", dest="shapenet_flag", action="store_true", help="dataset option")
	arg_parser.add_argument("--gpu", "-g", dest="gpu", default=0, help="gpu id")
	arg_parser.add_argument("--init_weight", dest="init_weight", default="initial", help="초기 weight 파일명(확장자 .pth 없이, 예: initial_abc)")
