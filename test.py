...
if __name__ == "__main__":
	...
	arg_parser.add_argument("--experiment", "-e", dest="experiment_directory", default="abc_voxel", help="The experiment directory which includes specifications and saved model files to use for reconstruction")
	arg_parser.add_argument("--checkpoint", "-c", dest="checkpoint", default="best_stage2", help="The checkpoint weights to use. This can be a number indicated an epoch or 'latest' for the latest weights (this is the default)")
	arg_parser.add_argument("--grid_sample", dest="grid_sample", default=64, help="dataset option")
	arg_parser.add_argument("--start", dest="start", default=0, help="start shape index")
	arg_parser.add_argument("--end", dest="end", default=1, help="end shape index")
	arg_parser.add_argument("--mc_threshold", dest="mc_threshold", default=0.9, help="marching cube threshold")
	arg_parser.add_argument("--csg", dest="csg", action="store_true", help="output csg mesh.")
	arg_parser.add_argument("--gpu", "-g", default=0, help="gpu id")
	arg_parser.add_argument("--phase", "-p", default="2", help="phase stage")
	arg_parser.add_argument("--shapenet", dest="shapenet", default=True, help="dataset option")
	arg_parser.add_argument("--test", dest="test_flag", default=True, help="test or train set shapes")
	arg_parser.add_argument("--code", dest="save_code", default=True, help="save shape code or not")
	arg_parser.add_argument("--primi_count", dest="primi_count", default=True, help="count primitives number or not")
	arg_parser.add_argument("--primi", dest="primi", default=True, help="output primitives or not")
	arg_parser.add_argument("--surface", dest="surface", action="store_true", help="dataset option")
	arg_parser.add_argument("--voxel", dest="voxel", default=True, help="dataset option")
